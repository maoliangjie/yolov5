...
class Detect(nn.Module):
    #Detect类负责yolov5的3个检测头层的构建， 对应模型配置文件models/yolov5s.yaml中的最后一层
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        #需要预测的box的维度， xywh+正样本置信度+80个类别每个类别的概率(coco是80个)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            #预测的box的中心点x和y是相对位置， 在推理的时候，需要映射到原图上，因此需要先加上grid的坐标，
            # 再乘以stride映射回原图。这里就是先把grid坐标计算出来。grid其实就是特征图的每一个点的坐标

                y = x[i].sigmoid()
                y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]  # xy
                #这里将预测框的中心点坐标转化到-0.5~1.5范围，然后将中心点映射回原图。至于为什么要将预测框的中心点坐标转化到-0.5~1.5范围，
                # 可以参考后面ComputeLoss代码解读的 __call__函数解读部分
